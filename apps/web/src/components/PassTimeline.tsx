...
export default function PassTimeline({ samples, minElevationDeg, satelliteName }: Props) {
  ...
  const peak = samples.reduce(
    (best, sample) => sample.elevation > best.elevation ? sample : best,
    samples[0] ?? { seconds: 0, elevation: 0, snr: 0 },
  );
  ...
  const durationLabel = lastVisible && lastVisible.seconds < (samples.at(-1)?.seconds ?? 0)
    ? t('LOS in ~{minutes} min', { minutes: Math.max(1, Math.ceil(lastVisible.seconds / 60)) })
    : t('Pass extends beyond window');

  return (
    <section className="panel pass-panel">
      <div className="panel-title-row">
        <div>
          <p className="eyebrow">{t('PASS PREDICTION')}</p>
          <h2>{satelliteName ?? t('No active pass')}</h2>
        </div>
        <span className="pass-status">{durationLabel}</span>
      </div>

      <div className="pass-summary">
        <div><span>{t('NOW')}</span><strong>{samples[0] ? `${samples[0].elevation.toFixed(1)}°` : '—'}</strong></div>
        <div><span>{t('PEAK')}</span><strong>{samples.length ? `${peak.elevation.toFixed(1)}°` : '—'}</strong></div>
        <div><span>{t('AT')}</span><strong>{samples.length ? `+${Math.round(peak.seconds / 60)}m` : '—'}</strong></div>
      </div>
      ...
      <div className="pass-chart-wrap">
        ...
        <div className="pass-axis">
          <span>{t('NOW')}</span><span>{t('+4m')}</span><span>{t('+8m')}</span><span>{t('+12m')}</span>
        </div>
      </div>
    </section>
  );
